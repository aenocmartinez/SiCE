...
<script>
    $(document).ready(function(){
        // Al cargar la vista con un convenio ya seleccionado
        checkearConvenio();
        ...
        // Formato de moneda en valorPago
        $('#valorPago').on('input blur', function() {
            formatCurrency(this);
        });

        // Eliminar formato antes del submit
    ...
    function checkearConvenio() {
        ...
        if (!nombreConvenio.toLowerCase().includes('cooperativa')) {
            $('#voucher').val('');
        }
    }
    ...
    function calcularDescuento(valorCosto, porcentajeDescuento) {
        return porcentajeDescuento ? Math.floor(valorCosto * (porcentajeDescuento / 100)) : 0;
    }

    function convertirFormatoCostoAEntero(valorCosto) {
        return parseInt(valorCosto.toString().replace(/[^0-9]/g, ''), 10) || 0;
    }
